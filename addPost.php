...
<div class="container-fluid text-white d-flex position-absolute top-50 start-50 translate-middle">
  <div class="form-div col-8 col-lg-4 mx-auto p-4">
    <h1 class="fw-semibold"> Add Post</h1>
      <form action="./server.php" method="POST" id="addPost" enctype="multipart/form-data">
        <div class="my-4">
          <input type="text" class="form-control  p-3 rounded-4 border-2 text-white <?= isset($errors['first_name']) ? 'border-danger' : 'border-white'  ?>  bg-transparent" id="title" placeholder="Title Name" name="title" >
        </div>
        <p class="text-danger"> <?= $errors['first_name'] ?? "" ?> </p>
        <div class="my-4">
          <input type="text" class="form-control  p-3 rounded-4 border-2  text-white <?= isset($errors['last_name']) ? 'border-danger' : 'border-white'  ?>  bg-transparent" id="content" placeholder="Content" name="content">
        </div>
        <p class="text-danger"> <?= $errors['last_name'] ?? "" ?> </p>
        <div class="my-4">
          <div class="w-50 mx-auto my-2 d-flex justify-content-center align-items-center overflow-hidden" style="aspect-ratio: 1">
              <img src="" id="post-img" class="w-100 h-100 object-fit-cover bg-white">
          </div>
          <input type="file" class="form-control  p-3 rounded-4 border-2 text-white bg-transparent" id="img" placeholder="Image url!" name="img" >
        </div>
...
$("#img").change(function (e) {
  ...
  $("#addPost").submit(function (e) {
    ...
    $.ajax({
      ...
      success: function(r) {
        alert(r);
        if (r == "The post has been uploaded successfully.") {
